...
def _find_disk_number_and_info():
    ...
    for line in lines:
        ...
        if match:
            ...
            # Not a disk-identifying line - if we are within an eligible disk's info block, accumulate info-lines;
            # else, move on
            if in_matching_disk:
                info += f'\n{line}'
    if disk_number == -1:
...
def _write_image_to_disk(image_file_path: Path, disk_number: int):
    ...
    try:
        while dd.poll() is None:
            ...
            if poll_result:
                line = dd.stderr.readline().decode('ascii')
                if 'records in' in line:
                    print(f'{line[:line.index("+")]} records, ', end='')
                if 'bytes' in line:
...
def _check_for_openssl_version():
    ...
    # if version_string
    if not version_string.startswith('OpenSSL') or (
            int(version_number.split('.')[0]) < 1 and int(version_number.split('.')[1]) < 1):
        print(
            f'Incorrect version of OpenSSL installed: "{version_string}". Check https://stackoverflow.com/a/62595252 '
            f'or https://ubuntu101.co.za/osx-and-macos/homebrew-missing-sha512sum-sha256sum-macos/ for more information')
        exit(1)
