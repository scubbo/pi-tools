...
echo
echo $'echo "curl -sfL https://get.k3s.io | sudo K3S_URL=https://$(ip addr | grep \'192.168\' | perl -pe \'s/.*inet (.*?)\\\/24.*/$1/\'):6443 K3S_TOKEN=$(sudo cat /var/lib/rancher/k3s/server/node-token) sh -s -"'
